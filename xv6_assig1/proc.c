...
// a helper function to help ps() system call because printing information about all processes requires access to ptable where in I can access each processe's PCB and this is only possible within 
// proc.c file so had to declare a extern ps function in sysproc.c (where the system call needs to be implemented) and defined it here. The idea was suggested in a stack overflow post.
void 
ps(void)
{
	struct proc *p;    // all these syntax taken from exit() system call inherent to xv6 
	acquire(&ptable.lock);
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
	{
 		 if( p->pid!=0) 
